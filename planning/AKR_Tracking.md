...
| Deliverable 11 - Artifact Normalization | Apply wording consistency pass — Round 1 (PHASE_0, PHASE_1, PHASE_2 plans): remove stale `.akr/ → validation/` migration language; update hook command paths to `$HOME/.akr/templates/` runtime-fetch model | Copilot | COMPLETE | Six targeted edits applied across three plan files; stale submodule migration wording replaced with runtime-fetch + local-cache terminology; hook VALIDATOR path updated from `.akr/templates/` to `$HOME/.akr/templates/` in Phase 1 plan examples and task acceptance criteria | 2026-04-04 | See Phase 2 Issues and Clarifications 2026-04-04 for architecture divergence note; existing Phase 2 hook tracking rows preserved |
| Deliverable 11 - Artifact Normalization | Apply wording consistency pass — Round 2 (PHASE_2_5, PHASE_2_6, PHASE_3, PHASE_4 plans): normalize KNOWN-GAP, HARD-GATE tokens; normalize em dashes and ≥/≤ threshold symbols; correct businessCapability tagging terminology | Copilot | COMPLETE | ~20 edits across four plan files; all KNOWN-GAP and HARD-GATE tokens uppercase; all `>=`/`<=` replaced with `≥`/`≤`; narrative dashes normalized to em dashes; "feature tagging" corrected to "businessCapability tagging" in PHASE_4 | 2026-04-04 | Verified via grep; zero residual stale phrases |
| Deliverable 13 - Post-Generation Fix Pass | Fix `BASELINE_REQUIRED_SECTIONS` in `akr_inline_validate.py` — two wrong heading strings caused false-positive section errors on every generated doc: `"Quick Reference"` → `"Quick Reference (TL;DR)"` and `"Questions and Gaps"` → `"Questions & Gaps"` | Copilot | COMPLETE | Updated both copies: `core-akr-templates/.github/skills/akr-docs/scripts/akr_inline_validate.py` and `training-tracker-backend/.github/skills/akr-docs/scripts/akr_inline_validate.py` | 2026-04-05 | Blocking prerequisite for reliable inline validation before re-run |
| Deliverable 13 - Post-Generation Fix Pass | Add explicit 🤖 marker mandate to `akr-generate.md` Step 6 — first generated draft produced unmarked AI narrative throughout because Step 6 had no enforcement rule | Copilot | COMPLETE | Added step 4 to Step 6 list: "every AI-inferred statement must include a 🤖 marker inline. Do not emit unmarked AI narrative." Applied to both canonical and app repo copies of `akr-generate.md` | 2026-04-05 | Corrects root cause of missing 🤖 markers in first Course draft |
| Deliverable 13 - Post-Generation Fix Pass | Add feature tag placeholder detection to `akr-generate.md` Step 1 — `FN00000_US000` placeholder from `modules.yaml` was copied verbatim into output front matter because the generate script had no detection rule | Copilot | COMPLETE | Added bullet to Step 1: "If `feature` value matches all-zero placeholder → write `feature: ❓ NEEDS real work-item tag` instead." Applied to both canonical and app repo copies of `akr-generate.md` | 2026-04-05 | Prevents placeholder propagation into committed draft and final docs |
| Deliverable 13 - Post-Generation Fix Pass | Add `akr_inline_validate.py` to `distribute-skill.yml` distribution manifest — packaging gap caused inline validator to be absent from app repos after skill distribution, breaking the automatic post-generation validation loop | Copilot | COMPLETE | Added cp command in distribute-skill.yml file sync block; updated PR body "Files Updated" list and "Reviewer Checklist" to include `akr_inline_validate.py`; `git add` already covers `.github/skills/akr-docs/scripts/` directory | 2026-04-05 | Will take effect on next `distribute-skill.yml` workflow execution |
| Deliverable 14 - Cache Management Architecture | Create `akr-cache.md` mode script — new 4th mode script handling `/akr-docs update-cache` and `/akr-docs cache-status` commands. Surfaced need: no mechanism existed to refresh `~/.akr/templates/` or report its state within the skill workflow. | Copilot | COMPLETE | Created `core-akr-templates/.github/skills/akr-docs/scripts/akr-cache.md` and distributed copy at `training-tracker-backend/.github/skills/akr-docs/scripts/akr-cache.md`. Covers: cache existence check, `git pull --ff-only`, changelog delta report, skill-version confirmation, age warning thresholds table. | 2026-04-05 | Prerequisite for all cache management commands. Added to `distribute-skill.yml` sync block. |
| Deliverable 14 - Cache Management Architecture | Add `--remote` flag to `generate` and `resolve` modes — no mechanism existed to force PATH A (live GitHub MCP REST call) and skip PATH B/C fallback. Users had to manually rename `~/.akr/templates/` to force remote fetch. | Copilot | COMPLETE | Updated `SKILL.md` dispatcher (canonical + app repo): description, Invocation Routing table rows, PATH Selection section, Token Budget note. Updated `akr-generate.md` (canonical + app repo): Step 2 and Step 3 fallback logic now hard-stops with failure report when `--remote` is present rather than falling back to local cache. | 2026-04-05 | Eliminates manual cache-rename workaround. Both canonical and app repo copies updated. |
| Deliverable 14 - Cache Management Architecture | Add passive cache health checks to `akr-generate.md` Step 1 pre-flight — generation runs had no visibility into whether the PATH B local cache was stale or version-mismatched relative to the distributed SKILL.md. | Copilot | SUPERSEDED | Added two checks to Step 1 of `akr-generate.md` (canonical + app repo): (1) skill-version comparison between distributed `.github/skills/akr-docs/SKILL.md` and `~/.akr/templates/SKILL.md`; (2) `FETCH_HEAD` mtime check with 30-day and 60-day advisory warning thresholds. Both checks are warnings only — generation is never blocked. Skipped entirely when `--remote` flag is present. | 2026-04-05 | SUPERSEDED 2026-04-05 by D15 architecture decision: end-user developers do not clone `core-akr-templates` locally; these checks referenced `~/.akr/templates/` which will not exist on developer workstations; both checks removed from `akr-generate.md` in the D15 correction pass. |
| Deliverable 15 - No-Local-Clone Architecture Correction | Architecture decision: end-user developers do NOT clone `core-akr-templates` locally. Only the select maintainer team manages any local cache. D14 cache distribution invalidated; PATH B redefined; hook validator gap resolved via Option A (distribute full validator). | Standards lead | COMPLETE | Decision documented in Phase 2 I&C 2026-04-05 (second entry). Cascades into 6 file changes and 2 file moves. | 2026-04-05 | Architectural reversal of D14 cache distribution items; selected Option A for hook validator gap. |
| Deliverable 15 - No-Local-Clone Architecture Correction | Roll back D14: move `akr-cache.md` from distributed scripts bundle to `core-akr-templates/docs/` as maintainer-only reference; delete app-repo copy. | Copilot | COMPLETE | `core-akr-templates/docs/akr-cache.md` created (moved from `.github/skills/akr-docs/scripts/`); `training-tracker-backend/.github/skills/akr-docs/scripts/akr-cache.md` deleted; `distribute-skill.yml` sync block updated to remove the cp command. | 2026-04-05 | `akr-cache.md` is a maintainer reference doc only — not distributed. |
| Deliverable 15 - No-Local-Clone Architecture Correction | Redefine PATH B in SKILL.md: remove `~/.akr/templates/` references; PATH B is now the workspace distributed copy at `.github/skills/akr-docs/scripts/`. Update Failure Handling to fall back to PATH B workspace copy (re-run `distribute-skill.yml` if absent). Remove `update-cache`/`cache-status` from routing table and description. | Copilot | COMPLETE | 5 changes applied to `core-akr-templates/.github/skills/akr-docs/SKILL.md` and 5 identical changes to `training-tracker-backend/.github/skills/akr-docs/SKILL.md`. | 2026-04-05 | PATH B is now unambiguous for end-user developers: workspace distributed copy, no local clone required. |
| Deliverable 15 - No-Local-Clone Architecture Correction | Remove D14 cache health checks and update Step 2/Step 3 fallbacks in `akr-generate.md`: PATH A failure is now a hard stop because templates and charters are never distributed to PATH B. | Copilot | COMPLETE | 3 changes applied to `core-akr-templates/.github/skills/akr-docs/scripts/akr-generate.md` and 3 identical changes to `training-tracker-backend/.github/skills/akr-docs/scripts/akr-generate.md`. | 2026-04-05 | Hard stop on PATH A failure is correct — templates and charters are PATH A only by design. |
| Deliverable 15 - No-Local-Clone Architecture Correction | Option A hook validator fix: distribute `validate_documentation.py` via `distribute-skill.yml` and update `agentStop.json` VALIDATOR path to workspace-relative `.github/skills/akr-docs/scripts/validate_documentation.py` in both repos. | Copilot | COMPLETE | `distribute-skill.yml`: removed `akr-cache.md` cp block, added `validate_documentation.py` cp block (sourced from `.akr/scripts/validate_documentation.py`), updated PR body and reviewer checklist. Both `agentStop.json` files updated: VALIDATOR path and not-found message corrected. | 2026-04-05 | Hook validation now works for all end-user developers without any local clone requirement. |
| Deliverable 15 - No-Local-Clone Architecture Correction | Update `PHASE_2_PILOT_ONBOARDING.md`: remove local cache init row, remove cache-status row, update confirm-skills row, remove mandatory cache init from Setup checklist, update Critical notes. | Copilot | COMPLETE | Onboarding checklist table: removed 2 rows; updated confirm-skills row (added `validate_documentation.py`, removed `akr-cache.md`); updated distribute-bundle row. Critical notes: replaced `~/.akr/templates/` mandatory note with no-local-clone note plus hook validation note. Setup checklist: removed 2 mandatory cache items; estimated time reduced to 2 hours. | 2026-04-05 | Onboarding now reflects the actual architecture: no per-developer cache setup required. |
...
- 2026-04-05: **Post-generation defect pass executed after first `/akr-docs generate Course` pilot run.** Four defects identified and resolved in this session:
  1. `akr_inline_validate.py` `BASELINE_REQUIRED_SECTIONS` had two wrong heading strings causing false-positive `[ERROR] Missing required section` failures on every generated doc. Root cause: heading text produced by generate step includes `(TL;DR)` suffix and `&` character, but baseline strings had neither. Fixed: `"Quick Reference"` → `"Quick Reference (TL;DR)"` and `"Questions and Gaps"` → `"Questions & Gaps"`. Both canonical and app repo copies updated.
  2. `akr-generate.md` Step 6 had no explicit 🤖 marker enforcement rule. The charter marker rules were only compressed into a ~400-token forward payload in Step 3 with no mandate in Step 6. Result: first Course draft produced unmarked AI narrative throughout with zero 🤖 markers (only 17 ❓ markers present). Fixed by adding step 4: "every AI-inferred statement must include a 🤖 inline". Both copies updated.
  3. `akr-generate.md` Step 1 had no detection for all-zero `feature` placeholder values. Result: `FN00000_US000` from `modules.yaml` was copied verbatim into the output front matter. Fixed by adding a pre-flight check: when `feature` matches the placeholder pattern, write `feature: ❓ NEEDS real work-item tag` instead. Both copies updated.
  4. `akr_inline_validate.py` was absent from `distribute-skill.yml` manifest (packaging gap discovered when inline validator failed to auto-trigger after generation). Fixed: added cp command to sync block; PR body and reviewer checklist updated. Will distribute on next workflow run.
- 2026-04-05: **`Course_draft.md` requires regeneration.** The existing draft at `training-tracker-backend/docs/modules/.akr/Course_draft.md` was generated before the fixes above. It contains: (a) verbatim `FN00000_US000` placeholder in front matter, (b) no 🤖 markers. Delete or overwrite via a fresh `/akr-docs generate Course` run. Inline validator will now pass section checks and correctly report the 17 ❓ markers as warnings.
- 2026-04-05: **Cache management architecture implemented (Deliverable 14).** Three capabilities added in this session following a user question about forcing a remote GitHub fetch when the local cache (`~/.akr/templates/`) may be stale:
  1. **`akr-cache.md` mode script (new):** Handles `/akr-docs update-cache` and `/akr-docs cache-status`. Operators can now refresh or inspect the local cache entirely within a skill invocation without manually running `git pull`. Distributed to both `core-akr-templates` and `training-tracker-backend`. Added to `distribute-skill.yml` for future propagation.
  2. **`--remote` flag:** Added to `generate` and `resolve` modes. When present, forces PATH A (live `@github` REST call) and hard-stops at any `@github` failure instead of falling back to the local cache. Eliminates the prior workaround of renaming `~/.akr/templates/` to force remote fetch.
  3. **Pre-flight cache health checks (passive):** `akr-generate.md` Step 1 now compares `skill-version:` between the distributed SKILL.md and the cache SKILL.md, and checks `FETCH_HEAD` mtime for advisory staleness warnings at 30-day and 60-day thresholds. Neither check blocks generation. Both checks are skipped when `--remote` is present. SKILL.md updated in both canonical and app repo copies to reflect new commands and flag.
- 2026-04-05: **Architecture decision and D14 correction (Deliverable 15): end-user developers do NOT clone `core-akr-templates` locally.** Critical analysis of the D14 implementation revealed a fundamental misalignment: D14 was designed around a `~/.akr/templates/` local cache that end-user developers were expected to maintain, but the actual architecture is that only the select maintainer team ever manages a local clone. CI GitHub Actions runners clone at runtime — unchanged. End-user developers use PATH A (`@github` live fetch) or PATH B (workspace distributed copy). The following corrections were applied in this session:
  1. **`akr-cache.md` moved to `docs/` (maintainer-only reference):** Removed from distributed skill bundle. `training-tracker-backend` copy deleted. `distribute-skill.yml` sync block updated.
  2. **PATH B redefined in SKILL.md (both repos):** Now unambiguously `.github/skills/akr-docs/scripts/` workspace distributed copy. All `~/.akr/templates/` references removed from PATH B and Failure Handling sections.
  3. **D14 cache health checks removed from `akr-generate.md` (both repos):** Both checks referenced `~/.akr/templates/` which will not exist on developer workstations. Hard stop on PATH A failure replaces soft fallback in Step 2/Step 3.
  4. **Hook validator gap resolved via Option A (distribute full validator):** `validate_documentation.py` added to `distribute-skill.yml` manifest, sourced from `.akr/scripts/validate_documentation.py`, distributed to `.github/skills/akr-docs/scripts/` in app repos. `agentStop.json` VALIDATOR path updated to workspace-relative `.github/skills/akr-docs/scripts/validate_documentation.py` in both repos. Hook validation now works without any local clone.
  5. **`--remote` flag semantics updated:** Now means force PATH A for mode script only. Templates and charters already require PATH A regardless, so `--remote` only affects the mode script fetch step.
  6. **`PHASE_2_PILOT_ONBOARDING.md` updated:** Local cache init and cache-status rows removed from checklist table. Confirm-skills row updated to reference `validate_documentation.py` (added) and drop `akr-cache.md` (removed). Setup checklist mandatory cache items removed. Critical notes updated to reflect no-local-clone architecture. Estimated setup time reduced from 3 hours to 2 hours.
  Deferred: `validate_documentation.py` drift between distributed copy (`.github/skills/akr-docs/scripts/`) and canonical (`.akr/scripts/`) is a known sync gap. Resolution deferred to a later session.
...
- 2026-04-04: **Seven implementation tasks completed (directive parser + review artifact fixes).**
  1. **`parse_template_directives.py` (base) — merged features from review version:** Added `required+condition` contradiction check and duplicate order sort-key check to `validate_directives()`. Added `_format_compact()` (compact JSON output without typed attributes) and `_validate_all()` (CI batch mode) functions. Expanded CLI with `--all DIR` (scan all `.md` in a directory) and `--compact` (compact JSON output) arguments. Base file preserves typed section schema, `--output text|json`, `format_text_output()`, and `"template": {}` JSON wrapper unchanged. File: `.akr/scripts/parse_template_directives.py`.
  2. **`validate-documentation-v2.yml` — Vale working-directory regression fixed:** Removed `cd ~/.akr/templates` (caused `$DOC_FILES` workspace-relative paths to resolve incorrectly) and the trailing `cd "$GITHUB_WORKSPACE"`. Vale now runs from `$GITHUB_WORKSPACE` throughout; `$VALE_CONFIG` already holds an absolute path so no config argument change was needed. File: `review/validate-documentation-v2.yml`.
  3. **`test_constants_sync.py` — path resolution made env-var-aware:** Replaced hardcoded `REPO_ROOT`-based `INLINE_VALIDATOR_PATH` and `FULL_VALIDATOR_PATH` with `os.environ.get()` overrides that fall back to `__file__`-relative defaults. CI can now set `INLINE_VALIDATOR_PATH` env var and the script will respect it. Fixed `VALID_COMPLIANCE_MODES` comparison to read from the full CI validator module via `getattr(full, "VALID_COMPLIANCE_MODES", ...)` instead of a hardcoded `frozenset`. File: `review/test_constants_sync.py`.
  4. **`akr_inline_validate_v2.py` — compliance mode detection corrected:** Changed `validate_file()` signature from `compliance_mode: str = "pilot"` to `compliance_mode: Optional[str] = None`. Updated internal logic: when `None`, reads from front matter; when front matter is absent or invalid, defaults to `"pilot"`; when explicitly set by caller, always uses the provided value. Removed the `compliance = args.compliance_mode or "pilot"` fallback in `main()` — now passes `args.compliance_mode` directly (preserving `None` for auto-detect). File: `review/akr_inline_validate_v2.py`.
  5. **`akr-resolve.md` — Phase 4 validator path fixed:** Replaced the full CI validator invocation (`python .akr/scripts/validate_documentation.py --file ... --fail-on needs`) with the correct inline validator invocation (`python .github/skills/akr-docs/scripts/akr_inline_validate.py {doc_path} --output text`) plus the pre-distribution fallback path. Added explanatory note that full CI validation runs only in CI after PR open. File: `review/akr-resolve.md`.
  6. **`akr-groupings.md` — Program.cs inclusion threshold clarified:** Replaced the vague "unless it contains significant DI logic" qualifier with a 3-criterion decision table (non-infrastructure `Add*` count ≥ 3; factory registrations for domain types; conditional domain service registration blocks). If `Program.cs` qualifies under any criterion it is added to a `Runtime` module; otherwise silently omitted. File: `review/akr-groupings.md`.
  7. **`akr-generate.md` — Step 2 fallback path and fetch instruction corrected:** Fixed PATH B from `.github/skills/akr-docs/scripts/` (incorrect — templates are not there) to `~/.akr/templates/templates/`. Since PATH B and PATH C now resolve to the same directory, consolidated to a single fallback: `~/.akr/templates/templates/ (local AKR cache)`. Corrected the "Fetch only" instruction to accurately describe the operation: fetch the full template file then parse and carry forward only the `akr:` directive blocks. File: `review/akr-generate.md`.
