...
VALIDATOR=".github/skills/akr-docs/scripts/validate_documentation.py"
...
if [ -n "$PYTHON_CMD" ]; then
  "$PYTHON_CMD" -c "from pathlib import Path; Path('.akr/logs').mkdir(parents=True, exist_ok=True)"
else
  mkdir -p .akr/logs
fi
...
CHANGED_FILES=$(git diff --name-only --diff-filter=AM HEAD -- docs)
if [ -n "$CHANGED_FILES" ]; then
  CHANGED_FILES="$CHANGED_FILES" "$VALIDATOR_PY" "$VALIDATOR" --changed-files --output json --fail-on needs | tee .akr/logs/last-validation.json
else
  "$VALIDATOR_PY" "$VALIDATOR" --all docs/modules --output json --fail-on needs | tee .akr/logs/last-validation.json
fi
