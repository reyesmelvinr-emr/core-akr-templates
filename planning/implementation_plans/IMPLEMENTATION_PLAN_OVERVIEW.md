...
| **Skill-as-scorer for semantic validation (PoC)** | New `/akr-docs score` mode computes semantic quality score in the developer's Copilot session — zero marginal API cost; mirrors `akr-resolve` pattern; score embedded in document YAML front matter (`semantic-score`, `semantic-scored-at`, `semantic-score-version`) before PR is opened; CI reads score from front matter and uploads per-section breakdown as ephemeral artifact (`retention-days: 7`); combined score formula: `(structural × 0.4) + (semantic × 0.6)`; PoC is advisory-only — never blocks merge; `SCORE_FRONT_MATTER_FIELDS` must never overlap with `DRAFT_ONLY_FIELDS` (see Part 22) |
